...
import { userBaseUrl, dataBaseUrl } from "./config.js";
...
function app() {
  ...
  async function onLogout() {
    await fetch(`${userBaseUrl}logout`, {
      method: "GET",
      headers: {
  ...
  async function onLoad() {
    ...
    const response = await fetch(`${dataBaseUrl}catches`);
  ...
  async function onUpdate(event) {
    ...
    if (angler && weight && species && location && bait && captureTime) {
      try {
        const response = await fetch(`${dataBaseUrl}catches/${event.target.dataset.id}`, {
          method: "PUT",
          headers: {
